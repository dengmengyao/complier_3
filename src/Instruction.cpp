...
Instruction::Instruction(unsigned instType, BasicBlock *insert_bb)
{
    ...
    if (insert_bb != nullptr&&insert_bb->brFlag==0)
    {
...
BinaryInstruction0::BinaryInstruction0(unsigned opcode, Operand *dst, Operand *src, BasicBlock *insert_bb) : Instruction(BINARY, insert_bb)
{//双目运算符
    this->opcode = opcode;
    operands.push_back(dst);
...
BinaryInstruction0::~BinaryInstruction0()
{
    operands[0]->setDef(nullptr);
    if(operands[0]->usersNum() == 0)
        delete operands[0];
...
void BinaryInstruction0::output() const{
...
void CondBrInstruction::output() const
{
    ...
    if(operands[0]->getType()->isInt()==1){
        new IntType(1);
        type=IntType(1).toStr();
    }
...
GlobalInstruction::GlobalInstruction(Operand *dst, SymbolEntry *se) : Instruction(GLOBAL)
{
    operands.push_back(dst);
    dst->setDef(this);
    this->se = se;
...
CallInstruction::CallInstruction(SymbolEntry *symbolentry,Operand *dst,vector<Operand*> vo, BasicBlock *insert_bb):Instruction(CALL,insert_bb){
    ...
    if(symbolentry->getType()->getRetType()->isInt()==1)
        u=0;
    
    else if(symbolentry->getType()->getRetType()->isVoid()==1)
       u=1;
    fprintf(stderr,"1\n");
};
